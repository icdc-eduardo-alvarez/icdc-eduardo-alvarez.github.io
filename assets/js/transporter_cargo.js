...
function init() {
  ...
  loader.load('./assets/models/TransporterCargo.glb', gltf => {
    let model = gltf.scene || gltf.scenes[0];
    let animations = gltf.animations || [];
    let animation = animations[0];
    mixer = new THREE.AnimationMixer(model);
    action = mixer.clipAction(animation).play();
    action.timeScale = animationDirection;
    totalTimeAnimation = animation.duration;
    scene.add(model);
    model.traverse(node => {
      if (node.isMesh) {
...
function raycast(e, touch = false) {
  ...
  if (intersects[0]) {
    ...
    if (object.name === 'SM_Right_Back_Door_0' || object.name === 'SM_Trunk_Top_1') {
      if(!isAnimating){
        play();
      }
    }
